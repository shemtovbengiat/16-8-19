...
void turnIni()
{
	//pinMode(turnL_Pin, INPUT_PULLUP);              // set the vinkers digital pin as output:
	//pinMode(turnR_Pin, INPUT_PULLUP);
...
void vinkers(bool turnLOn, bool turnROn, bool both)      // both = L+R fast together warnibg lights
{
	bool L = turnLOn;                      // for internal use                   
	bool R = turnROn;                      // for internal use                   
	bool B = both;					   // for internal use       

	if (L == 0 && R == 0)
	{
		digitalWrite(turnL_Pin, 1);      // 1=RELAY OFF ,pin at 0 activate relay by going ground ! upside down 
		digitalWrite(turnR_Pin, 1);           
	}
	else if (B == 0)                     // regular vinker - slow  
	{
		turnLRTiming = 200;              // 1400 slow 200 milliseconds)
		turnLights(L, R);                // service routine next peragaph 
	}
	else if (B == 1)                     // both vinkers  - emergency lights - fast 
	{
		turnLRTiming = 100;              // fast 60 milliseconds)
		turnLights(L, R);                // service routine next peragaph 
	}
...
void turnLights(bool L, bool R)
{
	...
	if (R == 1)
	{
		...
		if (currentRms - previousRms >= turnLRTiming)
		{
			...
			if (turnRState == 0) turnRState = 1;
			
			else turnRState = 0;     // 1 = RELAY OFF ,pin at 0 activate relay by going ground !!!!!!!!!!!!                                          vccccccccccccccccccc
		}
